...
temp_csv = fread(file.path('data','GLB.Ts+dSST.csv'))
temp_anomaly = select(temp_csv, "Year","D-N") # select yearly anomaly 
...
temp_history <- as.numeric(temp_anomaly[Year == 2020, "D-N"][[1]]) - as.numeric(temp_anomaly[Year == 1900, "D-N"][[1]])
