...
# Load popweighted country temperatureincrease in absolute value
files = Sys.glob(file.path("cmip5_temp","RegionalSCC_rcpfits","pop*.csv"))

all_ctemp = foreach(f=files) %do% {
  ...
  # Check Mongolia
  if(nrow(ctemp[Country=="Mongolia"])==0){
    .temp = ctemp[Country=="China"]
...
basetemp <- fread(file.path("cmip5_temp","popweightcountry_1980_2010_obsbaseline.csv"), header = F, skip = 0)
...
basetemp[Country=="Mongolia",temp:=basetemp[Country=="China",temp]]
